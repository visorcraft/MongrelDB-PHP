...
This document describes the security properties of the MongrelDB PHP client
and how to report vulnerabilities.

## Overview

The MongrelDB PHP client is a Composer library that talks to
`mongreldb-server` over HTTP. The client itself holds no encryption keys
and stores no data at rest; it is a thin request/response layer over the
daemon.
...
- Transport-specific behavior is isolated behind `TransportInterface`;
  server and network errors are mapped to the typed exception hierarchy
  (`AuthError`, `NotFoundError`, `ConflictError`, `QueryError`), not
  leaked as generic exceptions.
- NaN and Infinity are rejected with a clear `QueryError` rather than
  corrupting data.

## Dependency security

The MongrelDB PHP client has no runtime dependencies beyond PHP's standard
library (`ext-curl`, `ext-json`). All development dependencies are MIT or
Apache-2.0 licensed. Report dependency vulnerabilities through GitHub's
...
- the MongrelDB PHP client version, PHP version, and OS,
